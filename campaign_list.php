 ...
 *
 * Oeuvre derivee de htdocs/modulebuilder/template/myobject_list.php
 * de Dolibarr ERP/CRM (version 24.0.1).
 */

/**
 *   	\file       campaign_list.php
 *		\ingroup    salonerp
 *		\brief      List page for campaign
 */
...
include_once __DIR__.'/class/campaign.class.php';
// for other modules
//dol_include_once('/othermodule/class/otherobject.class.php');

// Load translation files required by the page
$langs->loadLangs(array("salonerp@salonerp", "other"));
...
$diroutputmassaction = $conf->salonerp->dir_output.'/temp/massgeneration/'.$user->id;
...
// List of fields to search into when doing a "search in all" (ref and label,
// the two fields marked 'searchall' in Campaign::$fields).
...
$permissiontodelete = $user->hasRight('salonerp', 'campaign', 'delete');
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Campaign';
	$objectlabel = 'Campaign';
	$uploaddir = $conf->salonerp->dir_output;
...
$title = $langs->trans("Campaigns");
//$help_url = "EN:Module_Campaign|FR:Module_Campaign_FR|ES:Módulo_Campaign";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	if ($obj) {
		$id = $obj->rowid;
		header("Location: ".dol_buildpath('/salonerp/campaign_card.php', 1).'?id='.((int) $id));
		exit;
	}
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-salonerp page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for a horizontal scroll in the table instead of page
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/salonerp/campaign_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, $object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendCampaignRef";
$modelmail = "campaign";
$objecttmp = new Campaign($db);
$trackid = 'campaign'.$object->id;
...
if ($search_all) {
	...
	print '<!-- Search done like if CAMPAIGN_QUICKSEARCH_ON_FIELDS = '.$setupstring.' -->'."\n";
...
// No document generation mass action in this module: no 'builddoc' block here.
