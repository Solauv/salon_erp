...
La validation génère une **clé privée**, affichée **une seule fois**, avec les boutons
« Copier » et « Télécharger (.key) ».
...
- **Onglet « Déchiffrer » de la campagne** (dès la validation) : déposez un fichier ; le
  module vérifie qu'il appartient à la campagne, qu'il est intact et qu'il correspond à la
  chaîne officielle. S'il diverge, il indique **le numéro du vote où les chaînes se
  séparent**. Une fois les votes révélés, il déchiffre aussi le fichier.
- **Menu Votes → Déchiffrement externe** : le fichier et la clé révélée suffisent ; rien
  n'est lu dans la base. Utilisable pour n'importe quelle campagne, y compris depuis une
  autre instance de Dolibarr où le module est installé.
...
Le fichier déposé est lu puis oublié : il n'est jamais conservé sur le serveur.
...
| `class/salonerpvotefile.class.php` | `SalonerpVoteFile` : analyse d'un fichier des votes, **sans accès à la base** |
| `campaign_card.php`, `campaign_list.php`, `campaign_note.php` | fiche, liste, notes |
| `campaign_vote.php` | onglet Voter : fichier des votes, formulaire de vote |
| `campaign_result.php` | onglet Résultats : révélation, résultats, attribution |
| `campaign_decrypt.php` | onglet Déchiffrer : contrôle d'un fichier contre la chaîne officielle |
| `decrypt_external.php` | déchiffrement externe et script autonome |
| `lib/salonerp_campaign.lib.php` | onglets, contrôle CSRF, lecture du fichier déposé, rendu du rapport, script autonome |
| `sql/` | tables et clés |
...
- Entrées via `GETPOST` typé ; sorties échappées ; aucun fichier déposé n'est écrit sur
  le disque (`is_uploaded_file`, lecture en mémoire).
- La page de la clé privée est servie avec `Cache-Control: no-store`.
